...
class Asset {
  protected _assetContainer: BABYLON.AssetContainer;

  constructor(protected _url: string, protected _meshPosition: BABYLON.Vector3,
    protected _meshRotation: BABYLON.Vector3, protected _meshScaling: BABYLON.Vector3,
    protected _materials: BABYLON.Material[], protected _scene: BABYLON.Scene) {
    if (!_materials) throw 'Materials list is empty!';
  ...
  get promise(): Promise<void> {
    return BABYLON.SceneLoader.ImportMeshAsync('', '', this._url, this._scene).then(result => {
      ...
      result.meshes.forEach(mesh => {
        ...
        if (materialIndex !== this._materials.length) materialIndex++;
      });
    });
  }
...
class AnimatedAsset extends Asset {
  protected _animationRanges = new Map<string, BABYLON.AnimationRange>();

  constructor(url: string, meshPosition: BABYLON.Vector3, meshRotation: BABYLON.Vector3, meshScaling: BABYLON.Vector3, materials: BABYLON.Material[], private _animationNames: string[], scene: BABYLON.Scene) {
    super(url, meshPosition, meshRotation, meshScaling, materials, scene);
    if (!this._animationNames) throw 'Animation names list is empty!';
  }

  get animationRanges(): Map<string, BABYLON.AnimationRange> {
...
class AssetClone {
  ...
  constructor(asset: Asset, protected _scene: BABYLON.Scene, isPickable = false) {
    this._assetContainer = new BABYLON.AssetContainer(_scene);

    if (this instanceof AnimatedAssetClone) return;
  ...
  set rotationQuaternion(rotationQuaternion: BABYLON.Quaternion) {
    this.meshes[0].rotationQuaternion = rotationQuaternion;
  }

  setAbsolutePosition(absolutePosition: BABYLON.Vector3): void {
  ...
  setParent(parent: BABYLON.Mesh, positionOffset: BABYLON.Vector3, rotation: BABYLON.Vector3): void {
...
class AnimatedAssetClone extends AssetClone {
  private _animationRanges = new Map<string, BABYLON.AnimationRange>();

  constructor(animatedAsset: AnimatedAsset, scene: BABYLON.Scene, isPickable = false) {
  ...
  animate(animationName: string, loop: boolean = true, callback?: (() => void)): void {
...
class Playground {
  public static CreateScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): BABYLON.Scene {
    ...
    let ybotTop: AnimatedAssetClone;
    let ybotBottom: AnimatedAssetClone;
    let ak47Collider: BABYLON.Mesh;
    let crossbowCollider: BABYLON.Mesh;
    let karambitCollider: BABYLON.Mesh;
    let marksmanCollider: BABYLON.Mesh;
    let pistolCollider: BABYLON.Mesh;
    let revolverCollider: BABYLON.Mesh;
    let rpgCollider: BABYLON.Mesh;
    let shotgunCollider: BABYLON.Mesh;
    let smgCollider: BABYLON.Mesh;
    let sniperCollider: BABYLON.Mesh;

    // const weapons = new Array<BABYLON.Mesh>();
    const weapons = new Map<number, BABYLON.Mesh>();
    const weaponPhysicsImposters = new Array<BABYLON.PhysicsImpostor>();
    ...
    const grenades = new Map<number, {
      grenade: BABYLON.InstancedMesh,
      bounceTime: number
    }>();

    // Obstacles include pillars, slide, and ground
    const obstacles = new Set<BABYLON.Mesh>();
    const obstaclePhysicsImposters = new Array<BABYLON.PhysicsImpostor>();
    ...
    gui.add(test, 'frameTime').name('frameTime').listen();

    const players = new Array<{
      ybotTop: AnimatedAssetClone,
      ybotBottom: AnimatedAssetClone,
      ak47Collider: BABYLON.Mesh,
      crossbowCollider: BABYLON.Mesh,
      // pistolCollider: BABYLON.Mesh,
      sniperCollider: BABYLON.Mesh,
    }>();

    let spineBone: BABYLON.Bone;

    // Loads all Assets in parallel
    const loadAssets = async (scene: BABYLON.Scene): Promise<void> => {
      ...
      ak47Material.freeze();
      const ak47Asset = new Asset(ak47URL,
        new BABYLON.Vector3(),
        new BABYLON.Vector3(),
      ...
      const crossbowAsset = new Asset(crossbowURL,
        new BABYLON.Vector3(),
        new BABYLON.Vector3(),
        new BABYLON.Vector3(1, 1, 1),
        [crossbowMaterial], scene);
      ...
      const karambitAsset = new Asset(karambitURL,
        new BABYLON.Vector3(),
        new BABYLON.Vector3(),
        new BABYLON.Vector3(10, 10, 10),
        [karambitMaterial], scene);
      ...
      const marksmanAsset = new Asset(marksmanURL,
        new BABYLON.Vector3(),
        new BABYLON.Vector3(),
        new BABYLON.Vector3(1, 1, 1),
        [marksmanMaterial], scene);
      ...
      const pistolAsset = new Asset(pistolURL,
        new BABYLON.Vector3(),
        new BABYLON.Vector3(),
        new BABYLON.Vector3(1, 1, 1),
        [pistolMaterial], scene);
      ...
      const sniperAsset = new Asset(sniperURL,
        new BABYLON.Vector3(),
        new BABYLON.Vector3(),
        new BABYLON.Vector3(1, 1, 1),
        [sniperMaterial], scene);
      ...
      const ybotTopAnimatedAsset = new AnimatedAsset(ybotTopURL, new BABYLON.Vector3(),
        new BABYLON.Vector3(0, 0, 0), new BABYLON.Vector3(1, 1, 1).scale(0.011),
        [ybotJointMaterial, ybotJointMaterial, ybotSurfaceMaterial],
        topNames, scene);
      const ybotBottomAnimatedAsset = new AnimatedAsset(ybotBottomURL, new BABYLON.Vector3(),
        new BABYLON.Vector3(0, 0, 0), new BABYLON.Vector3(1, 1, 1).scale(0.011),
        [ybotJointMaterial, ybotSurfaceMaterial],
        botNames, scene);
      ...
      ybotTop.setParent(compoundBody, new BABYLON.Vector3(0, 0, 0),
      ...
      ybotBottom.setParent(compoundBody, new BABYLON.Vector3(0, -1, 0),
        new BABYLON.Vector3(0, Math.PI / 2, 0));
      ...
      ak47Collider = BABYLON.MeshBuilder.CreateBox('ak47', { width: 1.1, height: 0.3, depth: 0.1 }, scene);
      ...
      sniper.setParent(sniperCollider, new BABYLON.Vector3(-0.24, 0.03, 0), new BABYLON.Vector3());
      sniper.show();
      sniperCollider.physicsImpostor = new BABYLON.PhysicsImpostor(sniperCollider,
        BABYLON.PhysicsImpostor.BoxImpostor, {
        mass: 1,
        restitution: 0,
        nativeOptions: {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        bottom2.physicsImpostor = new BABYLON.PhysicsImpostor(bottom2,
          BABYLON.PhysicsImpostor.SphereImpostor, {
          mass: 0,
        }, scene);
        middle2.physicsImpostor = new BABYLON.PhysicsImpostor(middle2,
          BABYLON.PhysicsImpostor.CylinderImpostor, {
          mass: 0,
        }, scene);
        top2.physicsImpostor = new BABYLON.PhysicsImpostor(top2,
          BABYLON.PhysicsImpostor.SphereImpostor, {
          mass: 0,
        }, scene);
        compoundBody2.physicsImpostor = new BABYLON.PhysicsImpostor(compoundBody2,
    ...
    let compoundBody2: BABYLON.Mesh;

    bottom.physicsImpostor = new BABYLON.PhysicsImpostor(bottom,
      BABYLON.PhysicsImpostor.SphereImpostor, {
      mass: 0,
    }, scene);
    middle.physicsImpostor = new BABYLON.PhysicsImpostor(middle,
      BABYLON.PhysicsImpostor.CylinderImpostor, {
      mass: 0,
    }, scene);
    top.physicsImpostor = new BABYLON.PhysicsImpostor(top,
      BABYLON.PhysicsImpostor.SphereImpostor, {
      mass: 0,
    }, scene);
    compoundBody.physicsImpostor = new BABYLON.PhysicsImpostor(compoundBody,
      BABYLON.PhysicsImpostor.NoImpostor, {
      mass: 1,
      friction: 0,
      // restitution: 0,
    ...
    cameraTargetMesh.position = new BABYLON.Vector3(0, 0.65, 0.75);
    ...
    slide.physicsImpostor = new BABYLON.PhysicsImpostor(slide,
      BABYLON.PhysicsImpostor.BoxImpostor, {
      mass: 0,
      // friction: 0,
      // restitution: 0.5,
    ...
    for (let i = 0; i < 100; i++) {
      ...
      pillar.physicsImpostor = new BABYLON.PhysicsImpostor(pillar,
        BABYLON.PhysicsImpostor.BoxImpostor, {
        mass: 0,
        // restitution: 0.5,
        nativeOptions: {
    ...
    interface Command {
      ...
      cameraBeta: number,
      fireKeyDown: boolean,
      pickupKeyDown: boolean
      throwGrenadeDown: boolean
    }
    ...
    scene.onAfterStepObservable.add(() => {
      ...
      test.frameTime = performance.now();
      ...
      players.forEach(player => {
        const { ybotTop, ybotBottom, ak47Collider, sniperCollider } = player;
    ...
    scene.registerBeforeRender(() => {
      ...
      if (pick.hit) {
        ...
        // const hitRadius = Math.min(cameraTargetPosition.subtract(pick.pickedPoint).length(), 1);
      ...
      players.forEach(player => {
        ...
        if (ybotTop !== undefined) {
          ...
          const weaponCollider = sniperCollider;
          ...
          otherBeta += dBeta;
          if (otherBeta > Math.PI || otherBeta < 0) dBeta *= -1;
    ...
    const throwGrenade = (clientFrameTime: number): void => {
      if (clientFrameTime - prevThrowTime > betweenThrowTime) {
        ...
        grenade.physicsImpostor = new BABYLON.PhysicsImpostor(grenade,
          BABYLON.PhysicsImpostor.BoxImpostor,
          {
            mass: 1e-10,
            restitution: 0.5,
    ...
    const move = (command: Command): void => {
      ...
      if (delta > 0) {
        if (delta < 10) {
          console.log(`%c Low valid delta: ${delta}`, 'color: #00ffff');
        } else if (delta > 20) {
          console.log(`%c High valid delta: ${delta}`, 'color: #ff00ff');
        } else {
          console.log(`%c Valid delta: ${delta}`, 'color: #00ff00');
        }
      } else {
        console.log(`%c Invalid delta: ${delta}`, 'color: #ff0000');
      }
    ...
    const smoke = (position: BABYLON.Vector3): void => {
    ...
    const flame = (position: BABYLON.Vector3): void => {
      const ps = new BABYLON.ParticleSystem('', 20, scene);
      ps.emitter = position;
